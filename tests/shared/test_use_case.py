...
from datacontest.shared import request_object as req, response_object as res
...
def test_use_case_cannot_process_valid_requests():
    ...
    assert response.message == \
        'NotImplementedError: process_request not implemented by UseCase'
...
def test_use_case_can_process_invalid_requests_and_returns_response_failure():
    ...
    invalid_request_object.add_error('someparam', 'error msg')
    ...
    assert response.message == 'someparam: error msg'
...
def test_use_case_can_manage_generic_exception_from_process_request():
    ...
    use_case.process_request.side_effect = TestException('error msg')
    response = use_case.execute(mock.Mock)

    assert not response
    assert response.type == res.ResponseFailure.SYSTEM_ERROR
    assert response.message == 'TestException: error msg'
